...
| Cost cannot run away                                 | **partly** — #66  | `TokenBudget` is checked before dispatch against a real `count_tokens` call and is wired into the eval harness, one budget per run. The half that is not built is the orchestrator stopping dispatch and reporting how many failures went unclassified.                                                                                                                              |
