...
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Agents never modify the working tree                 | The pure agent modules cannot import `fs` or `child_process` at all — `eslint.config.js` fails the build on it. `tests/unit/lint-guardrails.test.ts` feeds the config an actual violation, because a lint rule that silently does nothing passes exactly as loudly as one that works, and this one did for a while. The orchestrator writes exactly one file (`report.md`) and does so itself. |
